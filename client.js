(function () {
    ...
    // FIX 1: IDs correctos de modelos Groq
    const MODEL_ESTANDAR = "qwen/qwen3-32b";
    const MODEL_PRO = "qwen/qwen3-32b";
    const MODEL_VISION = "meta-llama/llama-4-scout-17b-16e-instruct";

    const KEYWORDS_PRO_CALCULO = ["impropia", "infinit", "converg", "diverg", "serie", "no corresponde", "falsa", "incorrecta"];
    ...
    const SYSTEM_CALCULO = [
        ...
        "2. PLANTEAMIENTOS DE ÁREA (CÁLCULO PURO, NO ESTADÍSTICA): El área geométrica bajo la curva $f(x)$ es estrictamente $\\int_a^b f(x) dx$. NUNCA multipliques la función por derivadas internas ni agregues coeficientes 0.5 o constantes mágicas para fabricar un 'área=1'.",
        "3. NO INVENTES problemas. Si te dan una gráfica o enunciado en una imagen, esa es la verdad absoluta.",
        ...
        "6. Tópicos Avanzados:",
        "   - Integrales Impropias: ¡ALERTA DE SIGNOS! Analiza algebraicamente a dónde tiende el exponente.",
        ...
        "7. CUIDADO CON PREGUNTAS NEGATIVAS: Si el enunciado dice 'NO corresponde', 'FALSA', o 'INCORRECTA', tu objetivo se INVIERTE. Busca el ÚNICO error matemático entre las opciones.",
        "8. ESTRICTAMENTE PROCEDIMIENTOS. No uses relleno de texto.",
        "",
        "ESTRUCTURA INQUEBRANTABLE:",
        "Tema: (Concepto teórico evaluado)",
        "Procedimiento: (Pasos algebraicos con LaTeX conectado por iguales)",
        "Resultado: (Formulación final numérica o integral sin resolver)",
        "Verificación: (Por qué la opción elegida es un espejo exacto del Resultado)",
        "---",
        "LETRA",
        "(La última línea OBLIGATORIAMENTE tiene SOLO UNA LETRA: A, B, C, D o E)"
    ].join(nl);

    // FIX 2: Leer texto de shadowRoot (web components d2l-html-block)
    ...
    // FIX 3: Navegar iframes D2L (document > ctl_2 > FRM_page)
    function getQuizDoc() {
        try {
    ...
    async function preguntarAI(enunciado, opciones, imagen) {
        ...
        if (KEYWORDS_PRO_CALCULO.some(k => enunciadoMinus.includes(k))) modeloAElegir = MODEL_PRO;
        ...
        // FIX 4: Rotación de múltiples keys
        const realizarPeticion = async (modeloParam) => {
            for (let i = 0; i < GROQ_KEYS.length * 2; i++) {
                ...
                try {
                    ...
                    // Extracción robusta de letra: última línea no vacía
    ...
    async function resolverPregunta(q, id) {
        try {
            ...
            // Enunciado: d2l-html-block anterior al fieldset
            ...
            // Opciones: d2l-html-block dentro de cada tr
            ...
            // Imagen: buscar en shadowRoot del enunciado
            let imgData = null;
            ...
            console.log(`[P${id}] Enunciado:`, enunciado.slice(0, 100));
            console.log(`[P${id}] Opciones (${opts.length}):`, opts.map(o => o.letra + ": " + o.texto.slice(0, 60)));
    ...
    // Procesar en serie con delay para no saturar TPM
