(async () => {
    ...
    const SYSTEM_CALCULO = [
        ...
        "REGLA DE ORO: Aplica trabajo-energía al SISTEMA COMPLETO. La tensión es fuerza INTERNA y se CANCELA.",
        "Solo quedan: gravitación (masa colgante) y fricción (masa horizontal).",
        "Ecuación SISTEMA: m_colgante·g·d − μ_k·m_horizontal·g·d = ½(m_total)·v²",
        "Donde m_total = m_colgante + m_horizontal, y d es la distancia que ambos se mueven.",
        "",
        "PATRÓN A — Hallar distancia d para alcanzar velocidad v (ambos se mueven juntos):",
        "  Ecuación: m_colg·g·d − μ_k·m_horiz·g·d = ½(m_colg + m_horiz)·v²",
        "  Despeje: d = (m_total·v²) / (2·g·(m_colg − μ_k·m_horiz))",
        "  ATAJO: También puedes hallar a = g·(m_colg − μ_k·m_horiz)/m_total y luego d = v²/(2a).",
        "  EJEMPLO: Josué 2m horizontal μ_k=0.04, María m cuelga, v=3.5 m/s:",
        "    a = 9.8·(m − 0.04·2m)/(3m) = 9.8·(1−0.08)/3 = 9.8·0.92/3 = 3.005 m/s²",
        "    d = 3.5²/(2·3.005) = 12.25/6.01 = 2.04 m → 2 m.",
        "",
        "PATRÓN B — Hallar μ_k cuando m₂ cae h, choca con el piso, y m₁ sigue d_extra más:",
        "  TRAMO A→B (ambos se mueven h): ½(m₁+m₂)·v_B² = m₂·g·h − μ_k·m₁·g·h",
        "  TRAMO B→C (solo m₁, la cuerda se afloja): ½m₁·v_B² = μ_k·m₁·g·d_extra",
        "    ⚠️ KE en B es SOLO de m₁: ½m₁·v_B² (NO ½(m₁+m₂)·v_B², porque m₂ ya se detuvo).",
        "  COMBINANDO (eliminar v_B²): μ_k = m₂·h / [(m₁+m₂)·d_extra + m₁·h]",
        "  EJEMPLO: m₁=19, m₂=10, h=2, d_extra=0.5 → μ_k = 20/[29·0.5+19·2] = 20/52.5 = 0.38.",
        "",
        "⛔ ERRORES FATALES EN ATWOOD+ENERGÍA (cometerlos cambia la respuesta):",
        "- Usar tensión en la ecuación de energía del SISTEMA. La T se cancela al sumar ambos cuerpos.",
        "- Poner signo NEGATIVO al trabajo de la tensión sobre la masa que es JALADA (la T va en dirección del movimiento → trabajo positivo).",
        "- Usar (m₁+m₂)·g·h para la PE cuando SOLO la masa colgante cae. La PE es m_colg·g·h.",
        "- Usar ½(m₁+m₂)·v² después de que una masa se detiene. Solo la masa que sigue moviéndose tiene KE.",
        "",
        ...
        "  · Atwood+fricción: usa las fórmulas de la sección ATWOOD arriba. Usa SISTEMA para cancelar T.",
        "  · ATAJO: si ya hallaste a con Newton, usa v²=2ad o v²=v₀²+2ad para hallar d directamente.",
